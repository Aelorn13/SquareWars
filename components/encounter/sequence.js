...
export const sequenceEncounter = {
  ...
  start(k, gameContext) {
    ...
    function blink(gameObject, blinkColor, finalColor) {
      ...
      k.tween(gameObject.color, blinkColor, 0.1, (c) => (gameObject.color = c), k.easings.linear).then(() => {
        k.tween(gameObject.color, originalColor, 0.1, (c) => (gameObject.color = c), k.easings.linear).then(() => {
