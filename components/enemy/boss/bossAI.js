...
export function attachBossBrain(k, boss, player, gameContext) {
  ...
  boss.speed = boss.speed * phase1.speedMultiplier;
  ...
  //   // --- Actions that should happen regardless of being busy ---
    
  //   // 1. Always rotate towards the player
  //   rotateTowardsPlayer(k, boss, player, 10);
  ...
  //   // --- Actions that should ONLY happen when NOT busy ---

  //   // If the boss is busy with an ability, we stop here.
  //   if (boss.isBusy) {
  //     return;
  //   }

  //   // 4. If not busy, decide on the next action (ability or move)
  //   const abilities = BOSS_CONFIG.phases[boss.phase].abilities;
  ...
  //     return; // Ability chosen, do nothing else this frame.
  //   }
  ...
  //   // 5. If no ability was chosen and we're not busy, perform the default action: move.
  //   boss.moveTo(player.pos, boss.speed);
  // });
