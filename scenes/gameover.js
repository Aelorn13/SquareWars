...
export function defineGameOverScene(k, getScore) {
  k.scene("gameover", (args = {}) => {
    k.add([k.text("GAME OVER"), k.anchor("center"), k.pos(k.width() / 2, k.height() / 2 - 100)]);
    ...
    k.add([k.text("Click to play again", { size: 24 }), k.anchor("center"), k.pos(k.width() / 2, k.height() / 2 + 110)]);
    let canClick = false;
